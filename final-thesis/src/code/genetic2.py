...
class Genetic(TSPSolver):
    ...
    def __init__(self, graph, g_args, conditions, absolute_mute=False):
        ...
        self._g_args_ = g_args
        self._chromosomes_ = g_args.get_chr()
        self._cr_ = g_args.get_cr()
        self._mr_ = g_args.get_mr()
        self._selection_method_ = g_args.get_method_select()
        self._crossover_method_ = g_args.get_method_cross()
        self._mutation_method_ = g_args.get_method_mutate()
        self._validate()

    def _validate(self):
        if self._chromosomes_ % 2 != 0:
