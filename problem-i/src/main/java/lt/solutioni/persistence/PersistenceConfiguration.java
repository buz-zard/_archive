package lt.solutioni.persistence;

import lt.solutioni.persistence.service.PersonRepositoryService;
import lt.solutioni.persistence.service.impl.PersonRepositoryServiceImpl;

import org.springframework.boot.orm.jpa.EntityScan;
import org.springframework.context.annotation.Bean;
import org.springframework.context.annotation.ComponentScan;
import org.springframework.context.annotation.Configuration;
import org.springframework.data.jpa.repository.config.EnableJpaRepositories;

/**
 * Persistence layer related configuration.
 * 
 * @author buzzard
 *
 */
@Configuration
@ComponentScan("lt.solutioni.persistence")
@EnableJpaRepositories(basePackages = "lt.solutioni.persistence.repository")
@EntityScan(basePackages = "lt.solutioni.persistence.domain")
public class PersistenceConfiguration {

    @Bean
    public PersonRepositoryService personRepositoryService() {
        return new PersonRepositoryServiceImpl();
    }

}
